...
class Lexicon(NamedTuple):
    ...
    about_bot: str = ('<b>Об этом боте:</b> \nНа данный момент в боте реализован следующий функционал: \nЧтобы получить '
                     ...
                     'песен доступны кнопки <b>"Аудио"</b> и <b>"YouTube"</b>, чтобы можно было послушать как оригиналы '
                     'песен, так и в переводе. Это помогает заучивать песни правильно! ☺️\nНадеемся, этот бот будет '
                     'большим благословением для вас. \n❗️ Если вы заметили ошибку, напишите, пожалуйста, '
                     f'разработчику 👨🏻‍💻: <b>{config.tg_bot.admin_username}</b>\n💳 Номер карты для пожертвований: '
                     f'<b>{config.card.card}</b>')

    not_found_by_num: str = ('Песня не найдена. 🤷\nНужно отправить боту номер песни '
                             f'(1-{config.amount_songs.amount_songs}) или фразу из песни. '
